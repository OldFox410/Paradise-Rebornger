...
public abstract partial class SharedGunUpgradeSystem
{
    ...
    {
        var uid = args.Shooter;
        if (uid == null)
            return;

        if (!TryComp<DamageableComponent>(uid.Value, out var comp))
            return;


        if (!HasComp<MobStateComponent>(args.Target))
            return;
        _damage.TryChangeDamage(new Entity<DamageableComponent?>(uid.Value, comp), ent.Comp.DamageOnHit);
    }
